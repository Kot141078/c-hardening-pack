...
def resolve_local_artifact_ref(ref: Any) -> tuple[dict[str, Any], Path] | None:
    ...
    if (
        ...
        or canonical_sha256(artifact) != ref.get("hash")
    ):
        return None
...
def semantic_memory_reliance(data: dict[str, Any]) -> list[ValidationIssue]:
    ...
    if (
        ...
        or registry_ref.get("hash") != canonical_sha256(registry_data)
    ):
        ...
        for field, (expected_kind, result_field) in expected_kinds.items():
            ...
            if (
                ...
                or canonical_sha256(target) != ref.get("hash")
            ):
...
def semantic_commit(data: dict[str, Any]) -> list[ValidationIssue]:
    ...
    if bool(previous) != bool(reason):
        issues.append(issue(
            ...
            "Changed-condition reevaluation must carry both previous_commit_record_ref and change_reason.",
        ))
...
def semantic_external_intake(data: dict[str, Any]) -> list[ValidationIssue]:
    ...
    elevated = {"FUNCTIONAL_ANALOG", "INTERFACE_ADAPTATION", "FORMAL_DEPENDENCY", "CODE_REUSE"}
    ...
    def resolve_artifact_ref(ref: Any) -> dict[str, Any] | None:
        ...
        if not (
            ...
            and canonical_sha256(artifact) == ref.get("hash")
        ):
            return None
        return artifact
    ...
    if relation in elevated:
        if license_status != "LICENSE_CLEARED_FOR_ADAPTATION":
            issues.append(issue("dependency_requires_license_clearance", "Any relation above TEST_SURFACE_CATALYST requires explicit license clearance."))
        if freeze != "PUBLIC_REFERENCE_VERIFIED" or not source.get("source_hash"):
            issues.append(issue("dependency_requires_verified_source_freeze", "Any relation above TEST_SURFACE_CATALYST requires a verified, hashed source freeze."))
        if not data.get("license_evidence_refs") or not data.get("dependency_evidence_refs"):
            issues.append(issue("dependency_requires_evidence", "An elevated relation requires license and actual local dependency evidence."))
        source_path = contained_path(ROOT, source.get("uri"))
        if not source_path or not source_path.is_file() or canonical_sha256(load_json(source_path)) != source.get("source_hash"):
            issues.append(issue("dependency_source_artifact_unresolved", "Elevated relation source must be a resolvable, hash-bound local frozen artifact."))
        source_artifact_path = contained_path(ROOT, source.get("uri"))
        frozen_source = load_json(source_artifact_path) if source_artifact_path and source_artifact_path.is_file() else {}
        source_artifact_id = frozen_source.get("artifact_id") if isinstance(frozen_source, dict) else None
        if not isinstance(frozen_source, dict) or any(
            frozen_source.get(field) != source.get(field)
            for field in ("author", "title", "published_at", "persistent_id")
        ):
            issues.append(issue("dependency_source_metadata_mismatch", "Elevated source author, title, publication time, and persistent identifier must match the hash-bound frozen source."))
        license_evidence = [resolve_artifact_ref(ref) for ref in data.get("license_evidence_refs", [])]
        dependency_evidence = [resolve_artifact_ref(ref) for ref in data.get("dependency_evidence_refs", [])]
        removal_evidence = resolve_artifact_ref(data.get("removal_breaks_evidence_ref"))
        if (
            frozen_source.get("evidence_scope") == "SYNTHETIC_NEGATIVE_FIXTURE_ONLY"
            or any(
                isinstance(evidence, dict)
                and evidence.get("evidence_scope") == "SYNTHETIC_NEGATIVE_FIXTURE_ONLY"
                for evidence in license_evidence + dependency_evidence
            )
            or (
                isinstance(removal_evidence, dict)
                and removal_evidence.get("evidence_scope") == "SYNTHETIC_NEGATIVE_FIXTURE_ONLY"
            )
        ):
            issues.append(issue(
                "negative_fixture_evidence_cannot_elevate_relation",
                "Synthetic negative-fixture evidence cannot establish an elevated external relation.",
            ))
        local_targets = set(data.get("local_target", []))
        dependency_tests = {
            evidence.get("verification_test")
        ...
        if (
            ...
            or any(
                evidence is None
                or evidence.get("evidence_kind") != "LICENSE_CLEARANCE"
                or evidence.get("source_artifact_id") != source_artifact_id
                or evidence.get("adaptation_cleared") is not True
                for evidence in license_evidence
            )
            or not dependency_evidence
            or any(
                evidence is None
                or evidence.get("evidence_kind") != "LOCAL_DEPENDENCY"
                or evidence.get("source_artifact_id") != source_artifact_id
                or evidence.get("dependency_present") is not True
                or evidence.get("local_target") not in local_targets
                or not local_path_with_fragment_resolves(evidence.get("verification_test"))
                for evidence in dependency_evidence
            )
            or removal_evidence is None
            or removal_evidence.get("evidence_kind") != "REMOVAL_BREAKS"
            or removal_evidence.get("source_artifact_id") != source_artifact_id
            or removal_evidence.get("removal_breaks") is not True
            or removal_evidence.get("local_target") not in local_targets
            or removal_evidence.get("local_test") not in dependency_tests
            or not local_path_with_fragment_resolves(removal_evidence.get("local_test"))
        ):
            issues.append(issue("dependency_evidence_unresolved", "Elevated relation evidence references must resolve by ID, version, path, and canonical hash."))
    if relation in elevated and not data.get("removal_breaks_evidence_ref"):
        issues.append(issue("formal_dependency_requires_removal_test", "FORMAL_DEPENDENCY or CODE_REUSE requires evidence that removal breaks the local proof or implementation."))
    if relation in {"COMPARISON_ONLY", "TEST_SURFACE_CATALYST", "NO_DEPENDENCY"} and not data.get("claim_ceiling"):
        issues.append(issue("bounded_relation_requires_claim_ceiling", "A bounded relation requires an explicit claim ceiling."))
    if relation in elevated:
        issues.append(issue(
            "elevated_relation_requires_manual_review",
            "Automated v0.1 acceptance is disabled for relations above TEST_SURFACE_CATALYST until an independent provenance trust root is available.",
        ))
    if relation == "CODE_REUSE":
        issues.append(issue("code_reuse_record_requires_manual_review", "CODE_REUSE records require manual review; automated acceptance is intentionally disabled."))
    ...
    if relation in {"COMPARISON_ONLY", "TEST_SURFACE_CATALYST", "NO_DEPENDENCY"} and (
    ...
    if relation in elevated and data.get("claim_ceiling_state") != "ELEVATED_RELATION_REQUIRES_PROOF":
        issues.append(issue("elevated_relation_claim_state_mismatch", "Elevated relations require the explicit proof-bound claim state."))
    if relation in elevated and (
        assertions.get("dependency_claimed") is not True
        or assertions.get("derivation_claimed") is not False
        or assertions.get("priority_claimed") is not False
        or assertions.get("ontology_transfer_claimed") is not False
        or (relation == "CODE_REUSE") != (assertions.get("code_reuse_claimed") is True)
    ):
        issues.append(issue(
            "elevated_relation_assertions_mismatch",
            "Elevated relations require a dependency assertion, relation-consistent code-reuse assertion, and no unsupported derivation, priority, or ontology-transfer claim.",
        ))
    unresolved_targets = []
    ...
    if passport_path:
        ...
        if not path or not path.is_file():
            ...
            if (
                ...
                or ref.get("hash") != canonical_sha256(passport)
            ):
            ...
            if (
                passport.get("repository") != "https://github.com/Kot141078/c-hardening-pack"
                or passport.get("base_sha") != "9a33e3866cde19939be22a903967bc94f566db76"
                or passport.get("reviewed_parent_sha") != "ead3fe6c69d99aafb84b8db98d3df4329ea3c918"
                or passport.get("passport_scope") != "CANDIDATE_FIXTURE_DERIVED_FROM_REVIEWED_PARENT"
            ):
                issues.append(issue("judge_corpus_passport_state_mismatch", "Corpus passport does not declare the fixed repository, base, reviewed parent, and candidate-fixture scope."))
            ...
            for source in source_artifacts:
                ...
                if domain == "CANONICAL_JSON_SHA256_V1":
                    actual_hash = canonical_file_hash(source_path)
                elif domain == "TEXT_LF_UTF8_SHA256":
                    actual_hash = normalized_text_sha256(source_path)
                else:
                    source_errors.append(str(relative))
                    continue
...
def semantic_continuity_history(data: dict[str, Any]) -> list[ValidationIssue]:
    ...
    if (
        data.get("schema_version") != "c-continuity-history-cases-0.1"
...
def semantic_carry_cost(data: dict[str, Any]) -> list[ValidationIssue]:
    ...
    if data.get("schema_version") != "c-continuity-carry-cost-profile-0.1" or not data.get("profile_id"):
...
def semantic_earth_bundle(
    ...
    if (
        data.get("schema_version") != "c-runtime-integrity-earth-test-0.1"
    ...
    if set(records) != {"decision_basis", "memory_reliance", "consequence_commit", "non_effect_witness"}:
        issues.append(issue("earth_structure_invalid", "Earth records must name exactly the four bound records."))
    loaded: dict[str, dict[str, Any]] = {}
    for name in ("decision_basis", "memory_reliance", "consequence_commit", "non_effect_witness"):
    ...
    nested_specs = {
        "decision_basis": ("urn:ivan-kotov:c-runtime-integrity:decision-basis-record:0.1", semantic_decision_basis),
        "memory_reliance": ("urn:ivan-kotov:c-runtime-integrity:memory-reliance-record:0.1", semantic_memory_reliance),
        "consequence_commit": ("urn:ivan-kotov:c-runtime-integrity:consequence-commit-record:0.1", semantic_commit),
        "non_effect_witness": ("urn:ivan-kotov:c-runtime-integrity:non-effect-witness-record:0.1", semantic_non_effect),
    }
    ...
    if (
        ...
        or decision_ref.get("hash") != canonical_sha256(decision)
    ):
    ...
    expected_memory_ref = (memory.get("record_id"), record_version(memory), canonical_sha256(memory))
    ...
    if (
        ...
        or witness_ref.get("hash") != canonical_sha256(witness)
    ...
    if (
        ...
        or later.get("old_decision_basis_hash") != canonical_sha256(decision)
        or later.get("old_consequence_commit_hash") != canonical_sha256(commit)
    ):
        issues.append(issue("earth_later_retry_not_rebound", "Any later retry must require new linked records while preserving old records."))
    return issues
...
def validate_registered_links(
    ...
    if record_type == "decision_basis_record":
        for ref in (data.get("basis") or {}).get("memory_reliance_refs", []):
            ...
            if (
                ...
                or ref.get("hash") != canonical_sha256(target)
            ):
                ...
                target,
                "urn:ivan-kotov:c-runtime-integrity:memory-reliance-record:0.1",
    ...
    if record_type == "consequence_commit_record":
        ...
        for ref in data.get("memory_reliance_refs", []):
            ...
            if (
                ...
                or ref.get("hash") != canonical_sha256(memory)
            ):
        ...
        if previous_ref:
            ...
            if (
                ...
                or previous_ref.get("hash") != canonical_sha256(previous)
                or linked_record_issues(
                    previous,
                    "urn:ivan-kotov:c-runtime-integrity:consequence-commit-record:0.1",
                    semantic_commit,
                )
                or parse_timestamp(previous.get("created_at")) is None
                or parse_timestamp(data.get("created_at")) is None
                or parse_timestamp(previous.get("created_at")) >= parse_timestamp(data.get("created_at"))
                or previous.get("governing_entity_id") != data.get("governing_entity_id")
                or (previous.get("target_effect") or {}).get("effect_id") != (data.get("target_effect") or {}).get("effect_id")
                or (previous.get("target_effect") or {}).get("target_ref") != (data.get("target_effect") or {}).get("target_ref")
            ):
                issues.append(issue("previous_commit_record_unresolved", "Changed-condition reevaluation must hash-bind a distinct registered prior consequence-commit record."))
    ...
    if record_type == "non_effect_witness_record":
        ...
        if (
            ...
            or commit.get("non_effect_witness_ref", {}).get("hash") != canonical_sha256(data)
...
def resolve_registered_evidence(
    ...
    if not isinstance(data, dict):
        return None
    actual_hash = canonical_sha256(data)
...
def main() -> int:
    ...
    for entry in entries:
        matched, issues = validate_fixture(entry, schemas, registry, record_registry, evidence_registry)
        path = entry["path"]
