...
def main() -> int:
    ...
    r2_scenarios = manifest.get("r2_closure_scenarios", [])
    r2_ids = {
        item.get("id")
    ...
    def record_diagnostic(message: str, affected_ids: set[str], *, r2: bool = False) -> None:
        affected = set(affected_ids)
        diagnostics.append((affected, message))
        (r2_failed_ids if r2 else historical_failed_ids).update(affected)

    if len(scenarios) != 62 or manifest.get("recovered_scenario_count") != 62 or manifest.get("unrecovered_scenario_count") != 0:
        record_diagnostic("manifest must declare recovered=62 and unrecovered=0", historical_ids)
    ...
    if not isinstance(categories, dict) or not categories:
        record_diagnostic("manifest must declare non-empty category_scenarios", historical_ids)
        categories = {}
    category_ids = [
        scenario_id
    ...
    if not isinstance(r2_scenarios, list) or not r2_scenarios or len(r2_ids) != len(r2_scenarios):
        record_diagnostic(
    ...
    print(
        "RUNTIME_ADVERSARIAL_R1 scenarios=62 recovered=62 unrecovered=0 "
        f"pass={62 - historical_failed_count} fail={historical_failed_count} "
    ...
    total_failed_ids = (historical_failed_ids & historical_ids) | (r2_failed_ids & r2_ids)
    print(
        f"RUNTIME_ADVERSARIAL_TOTAL scenarios={62 + len(r2_ids)} "
        f"pass={62 + len(r2_ids) - len(total_failed_ids)} "
        f"failed_scenarios={len(total_failed_ids)} diagnostic_count={len(diagnostics)}"
    )
    for _, diagnostic in diagnostics:
        print(diagnostic, file=sys.stderr)
    return 0 if not total_failed_ids else 1
